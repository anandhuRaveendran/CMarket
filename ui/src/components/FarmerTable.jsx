...
function FarmerRequestHistory() {
  ...
  useEffect(() => {
      const fetchAllCredits = async () => {
          try {
            const response = await fetch('api/getCreditsByFarmer', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
              body: JSON.stringify({farmerId:'anan126'}),
            });
              if (response.success) {
                  console.log(response.json())
                  setCredits(response.data); // Set the fetched credits to state
              } else {
                  console.error('Error fetching credits:', response.message);
              }
          } catch (error) {
